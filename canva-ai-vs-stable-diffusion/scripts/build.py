...
SITE_TITLE     = "AI Tool Alternative Finder"
SITE_DESC      = "Discover free and cheaper alternatives to popular paid AI tools — with detailed comparisons. Save hundreds per year."
GA_ID          = "G-FGB481RVVS"   # swap for your AI Tool Finder GA4 property ID
BUILD_DATE     = datetime.now(timezone.utc).strftime("%B %d, %Y")
BUILD_DATE_ISO = datetime.now(timezone.utc).strftime("%Y-%m-%d")
...
def paid_key(c):
    """Derive URL-safe key from paid_tool name."""
    return c["slug"].split("-vs-")[0]

def free_key(c):
    """Derive URL-safe key from free_tool name."""
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .alt-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 18px; display: flex; flex-direction: column; gap: 10px; }
...
<footer>
  <div class="container">
    ...
    <a href="{root}stats/index.html">Stats</a> &nbsp;·&nbsp;
    <a href="{root}blog/index.html">Blog</a></p>
    <p style="margin-top:8px">Updated {BUILD_DATE} &nbsp;·&nbsp; $0/month to operate &nbsp;·&nbsp; Content for informational purposes only</p>
  </div>
</footer>"""

# ─────────────────────────────────────────────────────────────────────────────
# PAGE SHELL — now includes Schema.org, OG, Twitter Card, GA
# ─────────────────────────────────────────────────────────────────────────────

def page_shell(title, description, body, root="", canonical="", schema="", page_type="website", keywords=""):
    can_tag  = f'<link rel="canonical" href="{SITE_URL}/{canonical}" />' if canonical else ""
    kw_tag   = f'<meta name="keywords" content="{keywords}" />' if keywords else ""
...
def build_index():
    ...
    # Count unique paid tools for alternatives-to pages
...
<div class="card" data-cat="{c['category']}">
  ...
  <div style="display:flex;gap:6px;margin-top:auto">
    ...
    <a href="alternatives-to-{pk}/index.html" class="btn" style="flex:1;background:rgba(63,185,80,.15);color:var(--green);font-size:11px">All Alts</a>
  </div>
</div>"""
...
<div class="container">
  ...
  <div class="grid">{cards}</div>
    ...
    write("index.html", page_shell(
        SITE_TITLE, SITE_DESC, body,
        canonical="",
        schema=schema,
        keywords="AI tool alternatives, free AI tools, ChatGPT alternative, Midjourney alternative, GitHub Copilot alternative"
    ))
...
<div style="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:16px;margin:20px 0;">
  ...
  "@graph": [
    {{
      "@type": "Article",
      "headline": "{c['paid_tool']} vs {c['free_tool']} — Free AI Alternative",
      "description": "Is {c['free_tool']} a good free alternative to {c['paid_tool']}? Detailed comparison of pricing, features, and migration.",
    ...
    {{
      ...
      "itemListElement": [
        ...
        {{"@type": "ListItem", "position": 2, "name": "{c['category']}", "item": "{SITE_URL}/category-{c['category'].lower().replace(' ', '-').replace('&', 'and')}/"}},
        {{"@type": "ListItem", "position": 3, "name": "{c['paid_tool']} vs {c['free_tool']}", "item": "{SITE_URL}/{c['slug']}/"}}
      ]
    }}
  ]
}}'''
...
<div class="container" style="padding-top:24px">
  <p style="font-size:13px;color:var(--muted);margin-bottom:16px">
    <a href="../index.html">🤖 AI Tool Alternative Finder</a> /
  ...
  <p style="margin:20px 0"><a href="../index.html" style="color:var(--muted);font-size:13px">← View All Comparisons</a></p>
</div>"""

    desc = f"Is {c['free_tool']} a good free alternative to {c['paid_tool']}? {c['savings']}. Detailed comparison of pricing, features, and migration path."
    write(
        f"{c['slug']}/index.html",
        page_shell(
            ...
            schema=schema,
            page_type="article",
            keywords=f"{c['paid_tool']} alternative, free {c['paid_tool']} alternative, {c['free_tool']} vs {c['paid_tool']}, {c['paid_tool']} replacement"
        )
    )
...
def build_migration(c):
    ...
    # Parse migration text into numbered steps
    raw = c["migration"]
    steps = []
    ...
    # HowTo schema
    steps_schema = ", ".join(
        f'{{"@type":"HowToStep","text":"{s.replace(chr(34), chr(39))}"}}'
        for s in steps
    )
    schema = f'''{{
  "@context": "https://schema.org",
  "@type": "HowTo",
  "name": "How to Migrate from {c['paid_tool']} to {c['free_tool']}",
  "description": "Step-by-step guide to switching from {c['paid_tool']} to {c['free_tool']} and saving {c['paid_price']}.",
  "step": [{steps_schema}]
}}'''
...
<div class="container" style="padding-top:24px">
  ...
  <h1 style="font-size:1.8rem;margin-bottom:12px">How to Migrate from {c['paid_tool']} to {c['free_tool']}</h1>

  <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:24px">
    <span class="badge badge-green">💰 Save {c['paid_price']}</span>
    <span class="badge badge-blue">🖥️ {c['setup_difficulty']} Setup</span>
    <span class="badge badge-blue">⏱️ {c['setup_time']}</span>
    <span class="badge badge-blue">📅 {BUILD_DATE}</span>
  </div>
  ...
  <div style="background:rgba(63,185,80,.08);border:1px solid rgba(63,185,80,.3);border-radius:8px;padding:20px;margin-bottom:24px">
    ...
    <p style="margin-top:8px;font-size:14px">{c['paid_tool']} costs <strong>{c['paid_price']}</strong>. {c['free_tool']} is <strong>{c['free_price']}</strong>. That's {c['savings']}.</p>
  </div>
  ...
  <div style="display:flex;gap:12px;flex-wrap:wrap;margin:24px 0">
    <a href="{c['free_url']}" target="_blank" rel="noopener"
       style="background:rgba(63,185,80,.15);color:var(--green);padding:10px 20px;border-radius:6px;font-weight:600;font-size:14px;border:1px solid rgba(63,185,80,.3)">
      Set Up {c['free_tool']} →
    </a>
    <a href="../{c['slug']}/index.html"
       style="background:var(--surface);color:var(--accent);padding:10px 20px;border-radius:6px;font-weight:600;font-size:14px;border:1px solid var(--border)">
      Full Comparison →
    </a>
  </div>

  <p style="margin-top:24px;font-size:13px;color:var(--muted)">
    <em>Migration steps are AI-generated. Verify with official documentation before proceeding. Data sourced {BUILD_DATE}.</em>
  </p>

  <p style="margin-top:20px"><a href="../index.html" style="color:var(--muted);font-size:13px">← View All Comparisons</a></p>
</div>"""

    desc = f"Step-by-step guide to migrating from {c['paid_tool']} to {c['free_tool']}. Save {c['paid_price']}. Setup time: {c['setup_time']}."
    write(
        f"{slug}/index.html",
        page_shell(
            f"How to Migrate from {c['paid_tool']} to {c['free_tool']} ({BUILD_DATE})",
            desc, body,
            root="../",
            canonical=f"{slug}/",
            schema=schema,
            page_type="article",
            keywords=f"migrate {c['paid_tool']} to {c['free_tool']}, switch from {c['paid_tool']}, {c['paid_tool']} migration guide, {c['free_tool']} setup"
        )
    )
    return slug

# ─────────────────────────────────────────────────────────────────────────────
# ALTERNATIVES-TO PAGES  (new)
# ─────────────────────────────────────────────────────────────────────────────

def build_alternatives_to(paid_tool_name, paid_tool_comps):
    """One page per paid tool listing all free alternatives."""
    pk   = paid_key(paid_tool_comps[0])
    slug = f"alternatives-to-{pk}"
    cat  = paid_tool_comps[0]["category"]
    emoji = paid_tool_comps[0]["category_emoji"]

    alt_cards = ""
    items_schema = []
    for i, c in enumerate(paid_tool_comps, 1):
        fk = free_key(c)
        alt_cards += f"""
<div class="alt-card">
  <div class="alt-name">{c['free_tool']}</div>
  <div class="alt-price">{c['free_price']}</div>
  <div style="font-size:12px;color:var(--muted)">Setup: {c['setup_difficulty']} · {c['setup_time']}</div>
  <div style="font-size:13px;color:var(--muted);margin-top:4px">{c['verdict_switch'][:100]}...</div>
  <div class="alt-links">
    <a href="../{c['slug']}/index.html" class="btn-compare">Full Compare →</a>
    <a href="../migrate-{pk}-to-{fk}/index.html" class="btn-migrate">Migrate →</a>
  </div>
</div>"""
        items_schema.append(
            f'{{"@type":"ListItem","position":{i},"name":"{c["free_tool"]}","url":"{SITE_URL}/{c["slug"]}/"}}')
  ...
  "@graph": [
    {{
      "@type": "ItemList",
      "name": "Best Free Alternatives to {paid_tool_name}",
      "numberOfItems": {len(paid_tool_comps)},
      "itemListElement": [{", ".join(items_schema)}]
    }},
    {{
      "@type": "BreadcrumbList",
      "itemListElement": [
        {{"@type":"ListItem","position":1,"name":"Home","item":"{SITE_URL}/"}},
        {{"@type":"ListItem","position":2,"name":"Alternatives to {paid_tool_name}","item":"{SITE_URL}/{slug}/"}}
      ]
    }}
  ]
}}'''
...
<div class="container" style="padding-top:24px">
  <p style="font-size:13px;color:var(--muted);margin-bottom:16px">
    <a href="../index.html">🤖 AI Tool Alternative Finder</a> / Alternatives to {paid_tool_name}
  </p>

  <div style="font-size:12px;color:var(--muted);margin-bottom:8px">{emoji} {cat}</div>
  <h1 style="font-size:1.8rem;margin-bottom:12px">Best Free Alternatives to {paid_tool_name}</h1>
  <p style="color:var(--muted);margin-bottom:20px">
    We compared <strong>{len(paid_tool_comps)} free alternative{'s' if len(paid_tool_comps) > 1 else ''}</strong> to {paid_tool_name}
    — with pricing, setup difficulty, and step-by-step migration guides.
  </p>
  ...
  <div style="background:rgba(63,185,80,.08);border:1px solid rgba(63,185,80,.3);border-radius:8px;padding:16px;margin-bottom:24px">
    ...
    <p style="margin-top:6px;font-size:14px">{paid_tool_comps[0]['paid_tool']} costs {paid_tool_comps[0]['paid_price']}. Every alternative below is free or dramatically cheaper.</p>
  </div>

  <div class="alt-grid">{alt_cards}</div>

  <p style="margin-top:32px;font-size:13px;color:var(--muted)">
    <em>Data sourced {BUILD_DATE}. Verify current pricing at official websites before making decisions.</em>
  </p>
  <p style="margin-top:16px"><a href="../index.html" style="color:var(--muted);font-size:13px">← View All Comparisons</a></p>
</div>"""

    desc = f"The {len(paid_tool_comps)} best free alternatives to {paid_tool_name} in {BUILD_DATE[:4]}. Detailed comparisons with pricing, setup guides, and migration paths."
    write(
        f"{slug}/index.html",
        page_shell(
            f"Best Free Alternatives to {paid_tool_name} ({BUILD_DATE[:4]}) — AI Tool Alternative Finder",
            desc, body,
            root="../",
            canonical=f"{slug}/",
            schema=schema,
            keywords=f"alternatives to {paid_tool_name}, free {paid_tool_name} alternative, {paid_tool_name} replacement, open source {paid_tool_name}"
        )
    )
    return slug

# ─────────────────────────────────────────────────────────────────────────────
# BLOG  (new — auto-generated from comparison data)
# ─────────────────────────────────────────────────────────────────────────────

BLOG_POSTS = [
    {
        "slug": "chatgpt-alternatives-2024",
        "title": f"7 Free ChatGPT Alternatives That Are Good Enough in {BUILD_DATE[:4]}",
        "intro": "ChatGPT Plus costs $20/month. Here are 7 free alternatives that handle 90% of the same tasks — and when each one makes sense.",
        "comps": ["chatgpt-plus-vs-openrouter", "chatgpt-plus-vs-mistral-free", "chatgpt-plus-vs-google-gemini-free"],
        "tags": ["AI Chat", "ChatGPT", "Free"],
    },
    {
        "slug": "github-copilot-alternatives",
        "title": f"GitHub Copilot Is $10/Month — These Free Alternatives Are Almost As Good ({BUILD_DATE[:4]})",
        "intro": "GitHub Copilot charges $10–$19/month per developer. We tested three free alternatives that cover the same use cases.",
        "comps": ["github-copilot-vs-codeium", "github-copilot-vs-continue-dev", "cursor-ai-vs-continue-dev"],
        "tags": ["AI Coding", "Copilot", "Free"],
    },
    {
        "slug": "midjourney-alternatives",
        "title": f"Best Free Midjourney Alternatives in {BUILD_DATE[:4]} — Tested and Ranked",
        "intro": "Midjourney costs $10–$60/month. Stable Diffusion runs locally for free and generates comparable images. Here's the full breakdown.",
        "comps": ["midjourney-vs-stable-diffusion", "dalle3-vs-stable-diffusion", "adobe-firefly-vs-stable-diffusion"],
        "tags": ["AI Image", "Midjourney", "Stable Diffusion"],
    },
    {
        "slug": "free-ai-api-alternatives",
        "title": f"Stop Paying OpenAI API Bills — These Free Alternatives Work for Most Projects ({BUILD_DATE[:4]})",
        "intro": "OpenAI API costs add up fast. Ollama, Groq, and Together AI all offer free or dramatically cheaper inference. Here's when to use each.",
        "comps": ["openai-api-vs-ollama", "openai-api-vs-groq", "openai-api-vs-together-ai"],
        "tags": ["AI API", "OpenAI", "Free"],
    },
    {
        "slug": "elevenlabs-alternatives",
        "title": f"ElevenLabs vs Free TTS Alternatives: What Actually Works in {BUILD_DATE[:4]}",
        "intro": "ElevenLabs charges $5–$99/month for text-to-speech. Coqui TTS and Piper TTS run locally with no limits. Here's how they compare.",
        "comps": ["elevenlabs-vs-coqui-tts", "murf-ai-vs-coqui-tts"],
        "tags": ["AI Voice", "ElevenLabs", "TTS"],
    },
]
...
<div class="card" style="flex-direction:column;gap:10px">
  ...
  <div style="font-size:13px;color:var(--muted)">{post['intro'][:120]}...</div>
...
<header>
  <div class="container">
    <h1>📝 Blog</h1>
    <p style="color:var(--muted)">Practical guides on switching from expensive AI tools to free alternatives.</p>
  </div>
</header>
<div class="container">
  <div class="grid" style="padding-top:24px">{post_cards}</div>
</div>"""

    write("blog/index.html", page_shell(
        f"Blog — {SITE_TITLE}",
        "Practical guides on replacing paid AI tools with free alternatives.",
        blog_body, root="../",
        canonical="blog/",
        keywords="AI tool guides, free AI alternatives, AI tool comparison"
    ))
...
<div style="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:16px;margin-bottom:12px">
  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
    <div>
      <strong>{c['paid_tool']}</strong> <span style="color:var(--muted)">vs</span> <strong style="color:var(--green)">{c['free_tool']}</strong>
      <div style="font-size:12px;color:var(--muted);margin-top:4px">{c['savings']}</div>
    </div>
...
<div class="container" style="padding-top:24px">
  ...
  <p style="color:var(--muted);font-size:13px;margin-bottom:24px">📅 {BUILD_DATE} &nbsp;·&nbsp; 🤖 AI-assisted &nbsp;·&nbsp; 5 min read</p>
  ...
  <div style="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:20px;margin:24px 0">
    ...
    <p style="color:var(--muted);font-size:13px;margin-top:6px">Enter your team size and see exactly how much you save.</p>
...
# ─────────────────────────────────────────────────────────────────────────────
# CATEGORY INDEX PAGES  (new)
# ─────────────────────────────────────────────────────────────────────────────

def build_category_pages():
    for cat in CATEGORIES:
        cat_comps = [c for c in COMPARISONS if c["category"] == cat]
        emoji = cat_comps[0]["category_emoji"]
        slug  = "category-" + cat.lower().replace(" ", "-").replace("&", "and")
        ...
        for c in cat_comps:
            ...
            fk = free_key(c)
            rows += f"""
<div class="card" style="flex-direction:row;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px">
  <div>
    <div style="font-weight:600">{c['paid_tool']} vs <span style="color:var(--green)">{c['free_tool']}</span></div>
    <div style="font-size:12px;color:var(--muted);margin-top:4px">{c['savings']}</div>
  </div>
  <div style="display:flex;gap:6px">
    <a href="../{c['slug']}/index.html" class="btn" style="padding:6px 12px;font-size:12px">Compare →</a>
    <a href="../migrate-{pk}-to-{fk}/index.html" style="background:rgba(63,185,80,.15);color:var(--green);padding:6px 12px;border-radius:6px;font-size:12px;font-weight:600;border:1px solid rgba(63,185,80,.3)">Migrate →</a>
  </div>
</div>"""
...
<header>
  <div class="container">
    <h1>{emoji} {cat}</h1>
    <p style="color:var(--muted)">{len(cat_comps)} free AI tool comparisons in this category</p>
  </div>
</header>
<div class="container" style="padding:24px 20px 48px">
  <p style="margin-bottom:20px"><a href="../index.html" style="color:var(--muted);font-size:13px">← All Categories</a></p>
  <div style="display:flex;flex-direction:column;gap:12px">{rows}</div>
</div>"""

        write(f"{slug}/index.html", page_shell(
            f"{cat} AI Tool Alternatives — {SITE_TITLE}",
            f"Free alternatives to popular {cat} AI tools. {len(cat_comps)} detailed comparisons.",
            body, root="../", canonical=f"{slug}/",
            keywords=f"free {cat} AI tools, {cat} alternatives"
        ))
...
calc();
    ...
    write("savings-calculator/index.html", page_shell(
        ...
        keywords="AI tool savings, ChatGPT cost, AI subscription cost calculator"
    ))
...
def build_changelog():
  ...
  <td style="font-size:12px">
    <a href="../{c['slug']}/index.html">Compare →</a> &nbsp;
    <a href="../migrate-{pk}-to-{fk}/index.html" style="color:var(--green)">Migrate →</a>
  </td>
</tr>"""
    ...
    cat_summary = "".join(
        f'<div class="stat"><div class="num">{len(v)}</div><div class="lbl">{k}</div></div>'
...
<div class="container">
  <div style="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:20px;margin:24px 0">
    ...
    <ul style="padding-left:20px;color:var(--muted);font-size:14px;line-height:2">
      <li>All {len(COMPARISONS)} comparison pages refreshed with latest pricing</li>
      <li>All {len(COMPARISONS)} migration guide pages updated</li>
      <li>Alternatives-to pages regenerated for all paid tools</li>
      <li>Sitemap updated and submitted to Google Search Console</li>
    </ul>
  </div>
  <div class="stats" style="justify-content:flex-start;margin-bottom:24px">{cat_summary}</div>
  <h2>📄 All Comparisons — {BUILD_DATE}</h2>
  <table>
    <thead><tr><th>Category</th><th>Paid Tool</th><th>Free Alternative</th><th>Savings</th><th>Links</th></tr></thead>
    <tbody>{rows}</tbody>
  </table>
...
def build_stats():
    ...
    cat_rows = "".join(
        f"<tr><td>{next(x['category_emoji'] for x in v)} {k}</td><td>{len(v)} comparisons</td><td>{len(v)} migration guides</td></tr>"
        for k, v in sorted(by_cat.items())
    )

    # Count unique paid tools for alternatives pages
    unique_paid = len(set(c["paid_tool"] for c in COMPARISONS))
...
<header>
  <div class="container">
    ...
    <p>Live stats for AI Tool Alternative Finder — updated {BUILD_DATE}</p>
  </div>
</header>
<div class="container">
  ...
  <table>
    <thead><tr><th>Category</th><th>Comparisons</th><th>Migration Guides</th></tr></thead>
  ...
  <p style="margin-top:32px"><a href="../index.html">← Home</a></p>
    ...
    write("stats/index.html", page_shell(
        f"Stats — {SITE_TITLE}",
        "Statistics for AI Tool Alternative Finder — comparisons, migration guides, and coverage by category.",
        body, root="../", canonical="stats/"
    ))
...
<div class="container" style="max-width:720px;padding-bottom:48px">
  ...
  <p>The site is built by a Python script that generates static HTML, hosted on GitHub Pages at zero cost. GitHub Actions rebuilds and refreshes the site every day at 6 AM UTC automatically.</p>
  <h2 style="margin-top:24px">Why?</h2>
  <p>AI tool subscriptions add up fast — ChatGPT Plus, Copilot, Midjourney, ElevenLabs, and Jasper can easily cost $100–$300/month. Excellent free alternatives exist for almost every use case.</p>
  <h2 style="margin-top:24px">What pages are included?</h2>
  <p>For each comparison we generate: a full comparison page, a step-by-step migration guide, and an "alternatives to" page for the paid tool — giving Google multiple entry points to the same underlying content.</p>
  <p style="margin-top:32px"><a href="../index.html">← View All Comparisons</a></p>
</div>"""
    write("about/index.html", page_shell(
        f"About — {SITE_TITLE}",
        "About AI Tool Alternative Finder — a free directory of alternatives to paid AI tools.",
        body, root="../", canonical="about/"
    ))
...
<div class="container" style="max-width:720px;padding-bottom:48px">
  ...
  <p>AI Tool Alternative Finder does not directly collect personal data. Google Analytics (GA4) is used for aggregate traffic analysis. Google AdSense may be used to display ads — see Google's privacy policy for details.</p>
  <h2 style="margin-top:24px">Cookies</h2>
  <p>We use cookies for analytics (Google Analytics) and advertising (Google AdSense). You can opt out via your browser settings or Google's ad settings.</p>
  ...
  <p>All pricing and feature information is AI-generated for informational purposes. Verify current pricing at official vendor websites before making purchasing decisions.</p>
  <h2 style="margin-top:24px">Contact</h2>
  <p>For privacy concerns, please open an issue on our <a href="https://github.com/aiopentec/ai-tool-alternative-finder">GitHub repository</a>.</p>
  <p style="margin-top:32px"><a href="../index.html">← Home</a></p>
</div>"""
    write("privacy/index.html", page_shell(
        f"Privacy Policy — {SITE_TITLE}",
        "Privacy Policy for AI Tool Alternative Finder.",
        body, root="../", canonical="privacy/"
    ))
...
<div class="container" style="max-width:600px;padding-bottom:48px">
  ...
  <div style="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:24px;margin-top:20px">
    ...
    <p>The best way to suggest improvements is to open a GitHub Issue.</p>
  ...
  <p style="margin-top:32px"><a href="../index.html">← Home</a></p>
    ...
    write("contact/index.html", page_shell(
        f"Contact — {SITE_TITLE}",
        "Contact AI Tool Alternative Finder.",
        body, root="../", canonical="contact/"
    ))

# ─────────────────────────────────────────────────────────────────────────────
# SITEMAP  (now includes migration + alternatives-to pages)
# ─────────────────────────────────────────────────────────────────────────────

def build_sitemap(migration_slugs, alt_slugs, blog_slugs, cat_slugs):
    urls = [
    ...
    # Category pages
    for s in cat_slugs:
        urls.append(f"  <url><loc>{SITE_URL}/{s}/</loc><changefreq>weekly</changefreq><priority>0.6</priority><lastmod>{BUILD_DATE_ISO}</lastmod></url>")
...
<div style="text-align:center;padding:80px 20px">
  ...
  <p style="color:var(--muted);margin:16px 0 32px">This page doesn't exist. The comparison you're looking for may have moved.</p>
...
def main():
    # Unique paid tools → group comparisons for alternatives-to pages
    paid_groups = {}
    for c in COMPARISONS:
    ...
    estimated = 1 + len(COMPARISONS) + len(COMPARISONS) + unique_paid + len(BLOG_POSTS) + 1 + len(CATEGORIES) + 8
    ...
    print(f"\n📦 Building {len(COMPARISONS)} migration guide pages...")
    migration_slugs = []
    for c in COMPARISONS:
        migration_slugs.append(build_migration(c))

    print(f"\n🎯 Building {unique_paid} alternatives-to pages...")
    alt_slugs = []
    for paid_name, comps in paid_groups.items():
        alt_slugs.append(build_alternatives_to(paid_name, comps))

    print(f"\n📝 Building blog ({len(BLOG_POSTS)} posts)...")
    build_blog()
    blog_slugs = [p["slug"] for p in BLOG_POSTS]

    print(f"\n🗂️  Building {len(CATEGORIES)} category index pages...")
    build_category_pages()
    cat_slugs = [
        "category-" + cat.lower().replace(" ", "-").replace("&", "and")
        for cat in CATEGORIES
    ]
    ...
    print("\n🗺️  Building sitemap...")
    build_sitemap(migration_slugs, alt_slugs, blog_slugs, cat_slugs)

    total = (1 + len(COMPARISONS) + len(migration_slugs) + len(alt_slugs)
